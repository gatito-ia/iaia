...
%% 1. Crear el sistema difuso Sugeno
fis = sugfis('Name', 'ControlInvernadero_Sugeno');

%% Entradas (igual que en Mamdani)
% Entrada 1: Temperatura (°C)
fis = addInput(fis, [0 40], 'Name', 'Temperatura');
fis = addMF(fis, 'Temperatura', 'trapmf', [0 0 15 20], 'Name', 'Fria');
fis = addMF(fis, 'Temperatura', 'gaussmf', [5 25], 'Name', 'Optima');
...
%% Salidas (ahora son constantes o lineales)
% Salida 1: Ventilacion (%)
fis = addOutput(fis, [0 100], 'Name', 'Ventilacion');
% Funciones constantes para Sugeno
fis = addMF(fis, 'Ventilacion', 'constant', 15, 'Name', 'Minima');
fis = addMF(fis, 'Ventilacion', 'constant', 50, 'Name', 'Media');
fis = addMF(fis, 'Ventilacion', 'constant', 85, 'Name', 'Maxima');

% Salida 2: Riego (%)
fis = addOutput(fis, [0 100], 'Name', 'Riego');
fis = addMF(fis, 'Riego', 'constant', 10, 'Name', 'Nulo');
fis = addMF(fis, 'Riego', 'constant', 30, 'Name', 'Leve');
fis = addMF(fis, 'Riego', 'constant', 80, 'Name', 'Fuerte');

%% Definir las reglas (similar pero con salidas constantes)
rule1 = "Temperatura==Fria & Humedad==Seco & Luz==Baja => Ventilacion=Minima, Riego=Nulo (1)";
rule2 = "Temperatura==Optima & Humedad==Normal & Luz==Moderada => Ventilacion=Media, Riego=Leve (1)";
rule3 = "Temperatura==Caliente & Humedad==Humedo & Luz==Alta => Ventilacion=Maxima, Riego=Fuerte (1)";
rule4 = "Temperatura==Fria & Humedad==Humedo => Ventilacion=Minima, Riego=Fuerte (1)";
...
%% Ver reglas
disp('Reglas del sistema difuso Sugeno');
showrule(fis)

%% Versión con Salidas Lineales (Opcional)
% Cambiar las salidas a funciones lineales
% Formato: [coeficientes de entrada + término constante]
% Ejemplo: a*Temperatura + b*Humedad + c*Luz + d

% % Salida 1: Ventilacion (%)
% fis = removeMF(fis, 'Ventilacion', 'all');
% fis = addMF(fis, 'Ventilacion', 'linear', [-1 0.5 0.2 15], 'Name', 'Minima');  % -1*Temp + 0.5*Hum + 0.2*Luz + 15
% fis = addMF(fis, 'Ventilacion', 'linear', [0.5 0.3 0.1 50], 'Name', 'Media');
% fis = addMF(fis, 'Ventilacion', 'linear', [2 -0.5 0.3 85], 'Name', 'Maxima');
% 
% % Salida 2: Riego (%)
% fis = removeMF(fis, 'Riego', 'all');
% fis = addMF(fis, 'Riego', 'linear', [0.1 0.8 0 10], 'Name', 'Nulo');
% fis = addMF(fis, 'Riego', 'linear', [0.3 0.5 0.1 30], 'Name', 'Leve');
% fis = addMF(fis, 'Riego', 'linear', [0.5 1.5 -0.2 80], 'Name', 'Fuerte');
% Las reglas se mantendrían igual, pero ahora las salidas serían combinaciones lineales de las entradas.